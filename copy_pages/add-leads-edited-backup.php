<?php
  ...
  {  
    ...
    // ************************************ This is for CE without location ************************************
    $insertedLeadIds = $SubmittedLeadIdsList;
    ...
    if($totalCustomerExuctiveCount > 0) {
        ...
        // var_dump($leadsCount); 
        // var_dump($totalCustomerExuctiveCount); 
        // var_dump(($leadsCount/$totalCustomerExuctiveCount)); 
        ...
        if($totalCustomerExuctiveCount < $leadsCount) {
            ...
            for ($i=0; $i < count($customerExecutiveArray); $i++) { 
                  ...
                  $admin_id = $customerExecutiveArray[$i]["admin_id"];
                  $employee_id = $customerExecutiveArray[$i]["employee_id"];                    
                  $location_id = implode(",", $customerExecutiveArray[$i]["location_id"]);                 
                  $employee_name = $customerExecutiveArray[$i]["employee_name"];
            ...
            $submittedLeadsArray = array($insertedLeadIds);
            
            if($remaining_leads > 0) {
              $remainingLeadsForLoop = $remaining_leads;
              foreach($submittedLeadsArray as $submittedLeadInfo) {                      
                $i1 = 0;
                $employeewiseLeadId = $customerExecutiveArray[$i1];
                // foreach($submittedLeadInfo as $employeewiseLeadId) 
                $i2 = 1;
                foreach($customerExecutiveArray as $singleEmployee) 
                {
                    if($i2 == 1 && $remaining_leads != 0) {
                      var_dump($submittedLeadInfo[$i1]);
                      $lead_id = $submittedLeadInfo[$i1]["lead_id"];
                      $admin_id = $singleEmployee["admin_id"];
                      $employee_id = $singleEmployee["employee_id"];                    
                      $location_id = $submittedLeadInfo[$i1]["location_id"];                 
            ...
            if(($leadsCount - $leadsAssigned) == 0) {
              // echo "Links Are Assigned Properly for location -".implode($customerExecutiveArray[$i]["location_id"])."<br>";
            }
        }
    }
    // ************************************ This is for CE without location ************************************
