  ...
  $pdo = Database::connect();
    ...
    {  
        ...
        {        
            ...
            array_push($insertedLeadIds, $pdo->lastInsertId());
        }
          ...
          for ($i=0; $i < count($customerExecutiveArray); $i++) { 
              ...
              $employeewiseLeadIds = $employeewiseTotalLeadIds[$i];
                ...
                foreach($employeewiseLeadIds as $employeewiseLeadId) {
                  ...
                  $sql = "INSERT INTO `assign_leads`(`leads_id`, `admin_id`, `employee_id`, `employee_name`, `status`,`transfer_status`,`lead_date`, `added_on`) VALUES (?,?,?,?,?,?,?,?)";
                  $q = $pdo->prepare($sql);
                  $q->execute(array($lead_id, $admin_id, $employee_id, $employee_name, 'Active', $transfer_status, $lead_date, $added_on));
                  $leadsAssigned++;
                }
          }
